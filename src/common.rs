...
pub(crate) fn test_if_valid_server(host: &str, name: &str) -> ResultType<SocketAddr> {
    ...
    let res = if host.contains(':') {
        host
            .to_socket_addrs()?
            .next()
            .ok_or_else(|| Error::msg(format!("Invalid {} {}", name, host)))
    } else {
        format!("{}:{}", host, 0)
            .to_socket_addrs()?
            .next()
...
pub fn init_args(args: &str, name: &str, about: &str) {
    ...
    // Load .env
    if let Ok(env_file) = Ini::load_from_file(".env") {
        if let Some(section) = env_file.section(None::<String>) {
    ...
    if let Some(config) = matches.value_of("config") {
        if let Ok(cfg) = Ini::load_from_file(config) {
            if let Some(section) = cfg.section(None::<String>) {
                for (k, v) in section.iter() {
                    std::env::set_var(arg_name(k), v);
                }
            }
